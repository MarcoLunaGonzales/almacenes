...
<html>
    ...
    <body>
        <h3 align="center">SALIDA DE ALMACEN POR TRASPASO</h3>
        <br>
        <form target="_blank" id="form" name="form">
            <table align="center" width="90%">
                ...
                String codAlmacen=request.getParameter("codAlmacen")==null?"0":request.getParameter("codAlmacen");
                    ...
                    Traspasos traspasos = (Traspasos)request.getSession().getAttribute("traspasos");
                    SalidasAlmacen salidasAlmacen = traspasos.getSalidasAlmacen();
            ...
            <br>
                <%
                    ...
                    String consulta = " select s.COD_SALIDA_ALMACEN,s.NRO_SALIDA_ALMACEN,s.COD_LOTE_PRODUCCION,s.FECHA_SALIDA_ALMACEN,cp.nombre_prod_semiterminado, " +
                            " prp.NOMBRE_PRODUCTO_PRESENTACION,a.NOMBRE_AREA_EMPRESA,t.NOMBRE_TIPO_SALIDA_ALMACEN,S.OBS_SALIDA_ALMACEN " +
                            ",isnull(al.NOMBRE_ALMACEN,'') as almacenDestino,s.COD_TIPO_SALIDA_ALMACEN" +
                            ...
                            " left outer join TRASPASOS tr on tr.COD_SALIDA_ALMACEN=s.COD_SALIDA_ALMACEN"+
                            " left outer join ALMACENES al on al.COD_ALMACEN=tr.COD_ALMACEN_DESTINO" +
                ...
                if(rs1.next()){
                    ...
                    out.print(" <tr><td rowspan='4'><img src='../../img/cofar.png' /></td><td><b> Nro Salida : </b></td> <td> "+(rs1.getString("NRO_SALIDA_ALMACEN")==null?"":rs1.getString("NRO_SALIDA_ALMACEN"))+" </td><td><b>Lote: </b></td> <td> "+(rs1.getString("COD_LOTE_PRODUCCION")==null?"":rs1.getString("COD_LOTE_PRODUCCION"))+" </td> </tr>");
                    out.print(" <tr><td><b>Fecha de Salida: </b></td><td> "+(rs1.getDate("FECHA_SALIDA_ALMACEN")==null?"":sdf.format(rs1.getTimestamp("FECHA_SALIDA_ALMACEN")))+" </td> <td><b>Producto :</b></td> <td>"+(rs1.getString("nombre_prod_semiterminado")==null?"":rs1.getString("nombre_prod_semiterminado"))+"</td> </tr>");
                    ...
                    out.print(" <tr><td><b>Tipo Salida: </b></td><td>"+(rs1.getString("NOMBRE_TIPO_SALIDA_ALMACEN")==null?"":rs1.getString("NOMBRE_TIPO_SALIDA_ALMACEN"))+"</td> <td><b>"+(rs1.getInt("COD_TIPO_SALIDA_ALMACEN")==3?"Almacen Destino:":"&nbsp;")+"</b></td><td>"+(rs1.getInt("COD_TIPO_SALIDA_ALMACEN")==3?rs1.getString("almacenDestino"):"")+"</td> </tr> ");
                    out.print(" </table> ");
            ...
            <table  align="center" width="70%" class="outputText2" style="border : solid #f2f2f2 1px;" cellpadding="0" cellspacing="0">
                ...
                consulta = " select sad.COD_SALIDA_ALMACEN,m.CODIGO_ANTIGUO,m.COD_MATERIAL,m.NOMBRE_MATERIAL,u.COD_UNIDAD_MEDIDA,u.ABREVIATURA,sad.CANTIDAD_SALIDA_ALMACEN " +
                         ...
                         " where sad.COD_SALIDA_ALMACEN = '"+salidasAlmacen.getCodSalidaAlmacen()+"' " +
