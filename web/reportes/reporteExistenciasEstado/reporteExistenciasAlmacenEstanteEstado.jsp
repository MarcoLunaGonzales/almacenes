        ...
        try {
            ...
            <table width="70%" align="center" class="outputText0" style="border : solid #000000 1px;" cellpadding="0" cellspacing="0" >
                ...
                <tr class="">
                    ...
                    <th  style="border : solid #D8D8D8 1px" class="border"  bgcolor="#f2f2f2"  align="center"><b>Ambiente</b></th>
                    <th  style="border : solid #D8D8D8 1px" class="border"  bgcolor="#f2f2f2"  align="center"><b>Pasillo</b></th>
                    <th  style="border : solid #D8D8D8 1px" class="border"  bgcolor="#f2f2f2"  align="center"><b>Estante</b></th>
                ...
                String sql_mat = "select m.cod_material,m.nombre_material,m.CODIGO_ANTIGUO,u.ABREVIATURA from materiales m , UNIDADES_MEDIDA u where m.cod_material in (" + codMaterial + ") and u.COD_UNIDAD_MEDIDA=m.COD_UNIDAD_MEDIDA order by m.nombre_material  ";
                ...
                double cantidad=0d;
                String estanteAlmacen="";
                String fila="";
                String columna="";
                String nombreMaterial = "";
                String codAntiguoMaterial = "";
                while (rs_mat.next()) {
                    ...
                    consulta = " select ISNULL(sum(id.CANTIDAD_RESTANTE), 0) cantidad_r,isnull(E.NOMBRE_ESTANTE,'') NOMBRE_ESTANTE,isnull(amb.nombre_ambiente,'') nombre_ambiente,isnull(ID.FILA,'') FILA,isnull(ID.COLUMNA,'') COLUMNA,isnull(e1.nombre_estado_material,'') nombre_estado_material " +
                             " from INGRESOS_ALMACEN i" +
                             " inner join INGRESOS_ALMACEN_DETALLE_ESTADO id on id.COD_INGRESO_ALMACEN =" +
                             " i.COD_INGRESO_ALMACEN and i.COD_ESTADO_INGRESO_ALMACEN = 1 and" +
                             " i.fecha_ingreso_almacen <= '"+fecha+" 23:59:59'" +
                             " LEFT OUTER JOIN ESTANTE_AMBIENTE E ON E.COD_ESTANTE = ID.COD_ESTANTE" +
                             " left outer join ambiente_almacen amb on amb.cod_ambiente = e.cod_ambiente left outer join estados_material e1 on e1.cod_estado_material = id.cod_estado_material " +
                             " where id.COD_MATERIAL = '"+rs_mat.getString("cod_material")+"' and id.CANTIDAD_RESTANTE > 0.1 " +
                             "  and i.COD_ALMACEN = '"+codAlmacen+"'" +//and id.COD_ESTADO_MATERIAL = '"+estadoMaterial+"'
                             " GROUP BY amb.nombre_ambiente,E.NOMBRE_ESTANTE,ID.FILA,ID.COLUMNA,e1.nombre_estado_material ";
                    Statement stdetalle=con.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_READ_ONLY);
                    System.out.println("consulta cantidad "+consulta);
                    ResultSet res=stdetalle.executeQuery(consulta);
                    res.last();
                    int filas = 0;// res.getRow();
                    res.beforeFirst();
                    while(res.next())
                    {
                        cantidad=res.getDouble("cantidad_r");
                        estanteAlmacen=res.getString("nombre_estante");
                        fila=res.getString("fila");
                        columna=res.getString("columna");
                        out.print("<tr>");
                        out.print("<td height='30PX' class='border' align='right'  style='border : solid #D8D8D8 1px;PADDING:8PX' rowspan = '"+filas+"' >&nbsp;" + (rs_mat.getString("CODIGO_ANTIGUO") == null ? "" : rs_mat.getString("CODIGO_ANTIGUO")) + "</td>");
                        out.print("<td  class='border' style='border : solid #D8D8D8 1px;PADDING:8PX'  align='right' rowspan = '"+filas+"'  >&nbsp;" + rs_mat.getString("nombre_material") + "</td>");
                        out.print("<td  class='border' style='border : solid #D8D8D8 1px;PADDING:8PX'  align='right'  >&nbsp;" + rs_mat.getString("ABREVIATURA") + "</td>");
                        out.print("<td  class='border' style='border : solid #D8D8D8 1px;PADDING:8PX'  align='right'  >&nbsp;" + res.getString("nombre_ambiente") + "</td>");
                        out.print("<td  class='border' style='border : solid #D8D8D8 1px;PADDING:8PX'  align='right'  >&nbsp;" + estanteAlmacen + "</td>");
                        out.print("<td  class='border' style='border : solid #D8D8D8 1px;PADDING:8PX'  align='right'  >&nbsp;" + fila + "</td>");
                        out.print("<td  class='border' style='border : solid #D8D8D8 1px;PADDING:8PX'  align='right'  >&nbsp;" + columna + "</td>");
                        out.print("<td  align='right' style='border : solid #D8D8D8 1px;PADDING:8PX' class='border'   >&nbsp;" + formato.format(cantidad) + "</td>");
                        out.print("<td  align='right' style='border : solid #D8D8D8 1px;PADDING:8PX' class='border'   >&nbsp;" + res.getString("nombre_estado_material") + "</td></tr>");
