package reportes.solicitudSalidaAlmacen;
    ...
    <head>
        ...
        <script type="text/javascript" >
                function openPopup(f,url){
                    //window.open(url,'detalle'+Math.round((Math.random()*10000)),'top=50,left=200,width=800,height=600,scrollbars=1,resizable=1');
                    f.action=url;
                    f.submit();
                }
                function openPopup1(url){
                    //window.open(url,'detalle'+Math.round((Math.random()*10000)),'top=50,left=200,width=800,height=500,scrollbars=1,resizable=1');
                    //window.open(url,'detalle'+Math.round((Math.random()*10000)),'top=50,left=200,width=800,height=500,scrollbars=1,resizable=1');
                    ...
                    consulta = " select g.NOMBRE_GESTION,t.NOMBRE_TIPO_SALIDA_ALMACEN,e.NOMBRE_ESTADO_SOLICITUD_SALIDA_ALMACEN,(p.NOMBRE_PILA+' '+p.AP_PATERNO_PERSONAL+' '+p.AP_MATERNO_PERSONAL) NOMBRE_SOLICITANTE,ae.NOMBRE_AREA_EMPRESA " +
                             ...
                             " from SOLICITUDES_SALIDA s inner join GESTIONES g on g.COD_GESTION = s.COD_GESTION " +
                              ...
                              " inner join ALMACENES a on a.COD_ALMACEN = s.COD_ALMACEN " +
                               "inner join PRESENTACIONES_PRODUCTO pp on s.COD_PRESENTACION=pp.cod_presentacion" +
                               " inner join COMPONENTES_PROD cp on s.COD_COMPPROD= cp.COD_COMPPROD " +
