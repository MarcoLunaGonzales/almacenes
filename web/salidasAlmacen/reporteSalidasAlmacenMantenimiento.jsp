...
<%@page import="com.cofar.bean.SalidasAlmacen"%>
        ...
        <form target="_blank" id="form" name="form">
            <table align="center" width="90%">
               ...
               try{
                ...
                ManagedSalidaAlmacen managed=(ManagedSalidaAlmacen)Util.getSessionBean("ManagedSalidaAlmacen");
                Iterator i=managed.getSalidasAlmacenList().iterator();
                String codSalidaAlmacen="";
                while(i.hasNext())
                {
                    SalidasAlmacen salidas=(SalidasAlmacen)i.next();
                    if(salidas.getChecked())
                        {
                        codSalidaAlmacen=String.valueOf(salidas.getCodSalidaAlmacen());
                    }

                }
                //=request.getParameter("codSalidaAlmacen")==null?"0":request.getParameter("codSalidaAlmacen");
            ...
            <br>
                <%
                    ...
                    String consulta = " select s.COD_ALMACEN,isnull(sm.NRO_ORDEN_TRABAJO,0) as NRO_ORDEN_TRABAJO,s.COD_SALIDA_ALMACEN,s.NRO_SALIDA_ALMACEN,s.COD_LOTE_PRODUCCION,s.FECHA_SALIDA_ALMACEN,cp.nombre_prod_semiterminado, " +
                            ...
                            ",sm.FECHA_APROBACION,sm.FECHA_CAMBIO_ESTADOSOLICITUD,isnull(sm.COD_ESTADO_SOLICITUD_MANTENIMIENTO,0) as COD_ESTADO_SOLICITUD_MANTENIMIENTO" +
                    ...
                    consulta = " select s.COD_ALMACEN,isnull(sm.NRO_ORDEN_TRABAJO,0) as NRO_ORDEN_TRABAJO,s.COD_SALIDA_ALMACEN,s.NRO_SALIDA_ALMACEN,s.COD_LOTE_PRODUCCION,s.FECHA_SALIDA_ALMACEN,cp.nombre_prod_semiterminado, " +
                            " prp.NOMBRE_PRODUCTO_PRESENTACION,a.NOMBRE_AREA_EMPRESA,t.NOMBRE_TIPO_SALIDA_ALMACEN,aa.NOMBRE_ALMACEN,s.OBS_SALIDA_ALMACEN" +
                ...
                if(rs1.next()){
                    ...
                    out.print(" <table cellspacing='2' cellpadding='2' align='center' border='0' style='text-align:left' class='outputText1'> " +
                            ...
                            " <tr><th><b>Fecha Inicial: </b></th><th style='font-weight:normal'>  </th> " + //"+(rs1.getDate("FECHA_SOLICITUD_MANTENIMIENTO")==null?"":sdf.format(rs1.getDate("FECHA_SOLICITUD_MANTENIMIENTO")))+"
            ...
            <table  align="center" width="60%" class="outputText2" style="border : solid #f2f2f2 1px;" cellpadding="0" cellspacing="0">
                ...
                consulta = " select (select sum(id.CANT_TOTAL_INGRESO_FISICO) from INGRESOS_ALMACEN i, INGRESOS_ALMACEN_DETALLE id " +
                            " where i.COD_ALMACEN="+codAlmacen1+" and i.COD_ESTADO_INGRESO_ALMACEN<>2 and id.COD_MATERIAL=m.COD_MATERIAL " +
                            " and i.FECHA_INGRESO_ALMACEN <= '"+fecha+" 23:59:59' and i.COD_INGRESO_ALMACEN=id.COD_INGRESO_ALMACEN) as sumaCantidadIngreso," +
                            " (select sum(sd.CANTIDAD_SALIDA_ALMACEN) from SALIDAS_ALMACEN s, SALIDAS_ALMACEN_DETALLE sd " +
                             " where s.COD_SALIDA_ALMACEN=sd.COD_SALIDA_ALMACEN and s.COD_ALMACEN="+codAlmacen1+" and  " +
                             " sd.COD_MATERIAL=m.COD_MATERIAL and s.FECHA_SALIDA_ALMACEN <='"+fecha+" 23:59:59' and s.COD_ESTADO_SALIDA_ALMACEN<>2) as sumaCantidadSalida, "+
                ...
                consulta = " select (select sum(i.CANTIDAD_RESTANTE) from INGRESOS_ALMACEN_DETALLE_ESTADO i" +
                        "  inner join INGRESOS_ALMACEN ia on ia.COD_INGRESO_ALMACEN = i.COD_INGRESO_ALMACEN" +
                        "  and ia.COD_ESTADO_INGRESO_ALMACEN = 1 and ia.COD_ALMACEN = 4" +
                        "  where i.COD_MATERIAL = sad.COD_MATERIAL" +
                        "  and i.CANTIDAD_RESTANTE>0.1) cantidad_restante , "+
                        "  isnull(m.CODIGO_ANTIGUO,'') as CODIGO_ANTIGUO,sad.COD_SALIDA_ALMACEN,m.COD_MATERIAL,m.NOMBRE_MATERIAL" +
                        ",u.COD_UNIDAD_MEDIDA,u.ABREVIATURA,sad.CANTIDAD_SALIDA_ALMACEN,m.COSTO_UNITARIO " +
                         " from SALIDAS_ALMACEN_DETALLE sad  " +
                         " inner join materiales m on sad.COD_MATERIAL = m.COD_MATERIAL " +
                ...
                while (res.next()){
                    ...
                    "<td class='bordeNegroTd' align='right'>&nbsp;"+nf.format(res.getDouble("cantidad_restante"))+"</td>"+
                    //"<td class='bordeNegroTd' align='right'>&nbsp;"+nf.format(res.getDouble("sumaCantidadIngreso")-res.getDouble("sumaCantidadSalida"))+"</td>"+
            ...
            <table  align="center" width="60%" class="outputText2" style="border-top:solid #000000 1px;" cellpadding="0" cellspacing="0">
                <tr>
                    <td colspan="3" align="left"> <b>Glosa:</b><%=glosa%></td>
                </tr>
                <tr>
