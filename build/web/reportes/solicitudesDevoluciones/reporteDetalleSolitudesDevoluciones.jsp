    ...
    <body>
        ...
        <form target="_blank" id="form" name="form">
            ...
            String consultaCabe="select s.NRO_SALIDA_ALMACEN,cp.nombre_prod_semiterminado,a.NOMBRE_ALMACEN,s.COD_LOTE_PRODUCCION,  "+
                            ...
                            " sd.OBSERVACION ,isnull(ad.NOMBRE_ALMACEN,'') as almacenDestino,ae.NOMBRE_AREA_EMPRESA"
                    + " from SOLICITUD_DEVOLUCIONES sd inner join PERSONAL p on sd.COD_PERSONAL = p.COD_PERSONAL"+
                        ...
                        + " left outer join almacenes ad on ad.COD_ALMACEN=sd.COD_ALMACEN_DESTINO"
                        +" left outer join AREAS_EMPRESA ae on ae.COD_AREA_EMPRESA=s.COD_AREA_EMPRESA"
                    + " where sd.COD_SOLICITUD_DEVOLUCION='"+codSolicitud+"'";
            ...
            if(res.next()){
                    ...
                    out.print(" <tr><td rowspan='6'><img src='../../img/cofar.png' /></td><td><b> Nro Solicitud : </b></td> <td> "+(res.getString("COD_SOLICITUD_DEVOLUCION")==null?"":res.getString("COD_SOLICITUD_DEVOLUCION"))+" </td><td><b>Estado de Solicitud: </b></td> <td> "+(res.getString("NOMBRE_ESTADO")==null?"":res.getString("NOMBRE_ESTADO"))+" </td> </tr>");
                    out.print(" <tr><td><b>Solicitante: </b></td><td> "+(res.getString("nombrePer")==null?"":(res.getString("nombrePer")))+" </td> <td><b>Fecha Solicitud :</b></td> <td>"+(sdf.format(res.getTimestamp("FECHA_SOLICITUD")))+"</td> </tr>");
                     ...
                     out.print(" <tr><td><b>Producto: </b></td><td> "+(res.getString("nombre_prod_semiterminado")==null?"":(res.getString("nombre_prod_semiterminado")))+" </td> <td><b>Presentación:</b></td> <td>"+(res.getString("presentacion"))+"</td> </tr>");
                      out.print(" <tr><td><b>Lote: </b></td><td> "+(res.getString("COD_LOTE_PRODUCCION")==null?"":(res.getString("COD_LOTE_PRODUCCION")))+" </td><td><b>Area :</b></td><td>"+res.getString("NOMBRE_AREA_EMPRESA")+"</td></tr>");
                      out.print(" <tr><td><b>Almacen Destino: </b></td><td> "+(res.getString("almacenDestino"))+" </td><td></td><td></td></tr>");
            ...
            %>
            <table  class="tablaReporte" align="center" width="90%"  cellpadding="0" cellspacing="0">
                <thead>
                    <tr>
                        ...
                        <td  align="center" class="bordeNegroTd"><b>Cant. devuelta fallados proveedor</b></td>
                        <td  align="center" class="bordeNegroTd"><b>Unid.</b></td>
                    </tr>
                </thead>
                <%
                String conDetalle=" select isnull(m.CODIGO_ANTIGUO,'') as CODIGO_ANTIGUO,m.NOMBRE_MATERIAL,sdd.CANTIDAD_DEVUELTA,"+
                                            "sdd.CANTIDAD_DEVUELTA_FALLADOS,sdd.CANTIDAD_DEVUELTA_FALLADOS_PROVEEDOR,u.abreviatura "+
                                            ",sdd.COSTO_SOLICITUD_DEVOLUCION_FRV,sdd.COSTO_SOLICITUD_DEVOLUCION_FRV_PROVEEDOR"+
                                   " from SOLICITUD_DEVOLUCIONES_DETALLE sdd INNER JOIN MATERIALES m inner join unidades_medida u on u.cod_unidad_medida = m.cod_unidad_medida"+
                                        " on sdd.COD_MATERIAL=m.COD_MATERIAL"
                                    + " where sdd.COD_SOLICITUD_DEVOLUCION='"+codSolicitud+"'"
                ...
                while(res.next())
                 {
                    ...
                    <td  align="center" class="bordeNegroTd"><%=res.getString("CANTIDAD_DEVUELTA_FALLADOS")%><%=(res.getDouble("COSTO_SOLICITUD_DEVOLUCION_FRV") > 0  && res.getDouble("CANTIDAD_DEVUELTA_FALLADOS") > 0 ? "<br/>"+formato.format(res.getDouble("COSTO_SOLICITUD_DEVOLUCION_FRV"))+" (Bs)" : "")%></td>
                    <td  align="center" class="bordeNegroTd"><%=res.getString("CANTIDAD_DEVUELTA_FALLADOS_PROVEEDOR")%><%=(res.getDouble("COSTO_SOLICITUD_DEVOLUCION_FRV_PROVEEDOR") > 0  && res.getDouble("CANTIDAD_DEVUELTA_FALLADOS_PROVEEDOR") > 0 ? "<br/>"+formato.format(res.getDouble("COSTO_SOLICITUD_DEVOLUCION_FRV_PROVEEDOR"))+" (Bs)" : "")%></td>
                    <td  align="center" class="bordeNegroTd"><%=res.getString("abreviatura")%></td>
                    </tr>
                    <%
                }
                %>
