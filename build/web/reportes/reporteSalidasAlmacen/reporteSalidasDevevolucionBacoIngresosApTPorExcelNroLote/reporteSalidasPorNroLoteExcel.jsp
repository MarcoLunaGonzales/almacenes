            ...
            <table width="90%" align="center" class="outputText0 tablaReporte" cellpadding="0" cellspacing="0" >
                ...
                for(String codLote:arrayLotes)
                {
                    ponerColor=true;
                        ...
                        int contSalidaAlmacen=0;
                        String innerHTMLFinal="";
                        String innerHTMLInicio="";
                        while (rs.next()) {
                            ...
                            String innerHTMLTemporal="<tr bgcolor='"+codColumna+"'>"+
                            "<td    align='right'   >" + (rs.getString("NRO_SALIDA_ALMACEN") == null ? "" : rs.getString("NRO_SALIDA_ALMACEN")) + "</td>"+
                            "<td   align='right'   >" + (rs.getString("NOMBRE_TIPO_SALIDA_ALMACEN") == null ? "" : rs.getString("NOMBRE_TIPO_SALIDA_ALMACEN")) + "</td>"+
                            "<td      >" + format.format(rs.getDate("FECHA_SALIDA_ALMACEN")) + " &nbsp; " + format1.format(rs.getTimestamp("FECHA_SALIDA_ALMACEN").getTime()) + "</td>"+
                            "<td      >" + (rs.getString("COD_LOTE_PRODUCCION") == null ? "" : rs.getString("COD_LOTE_PRODUCCION")) + "</td>"+
                            "<td     >&nbsp;" + (rs.getString("CODIGO_ANTIGUO") == null ? "" : rs.getString("CODIGO_ANTIGUO")) + "</td>"+
                            "<td   >" + (rs.getString("NOMBRE_MATERIAL") == null ? "" : rs.getString("NOMBRE_MATERIAL")) + "</td>"+
                            "<td   >" + (rs.getString("NOMBRE_CAPITULO") == null ? "" : rs.getString("NOMBRE_CAPITULO")) + "</td>"+
                            "<td   >" + (rs.getString("NOMBRE_GRUPO") == null ? "" : rs.getString("NOMBRE_GRUPO")) + "</td>"+
                            "<td   >" + (rs.getString("ABREVIATURA") == null ? "" : rs.getString("ABREVIATURA")) + "</td>"+
                            "<td  ALIGN='RIGHT'   >" + (formato.format(rs.getDouble("cantidad_salida_almacen"))) + "</td>"+
                            "<td  ALIGN='RIGHT'   >" + (redondear(rs.getDouble("costo_salida"), 4)) + "</td>"+
                            "<td  ALIGN='RIGHT'   >" + (formato.format(rs.getDouble("total_salida"))) + "</td>"+
                            "<td  ALIGN='RIGHT'   >" + (formato.format(cantDev)) + "</td>"+
                            "<td  ALIGN='RIGHT'   >" + (redondear(costoUnitDev, 4)) + "</td>"+
                            "<td  ALIGN='RIGHT'   >" + (formato.format(costoTotalDev)) + "</td>"+
                            "<td  ALIGN='RIGHT'   >" + (formato.format(rs.getDouble("cantidad_salida_almacen")-cantDev)) + "</td>";
                            double costoUni=0d;
                            ...
                            innerHTMLTemporal+="<td  ALIGN='RIGHT'   >" + redondear(costoUni,4) + "</td>"+
                                             "<td  ALIGN='RIGHT'   >" + (formato.format(rs.getDouble("total_salida")-costoTotalDev)) + "</td>"+
                                             "<td    >" + rs.getString("nombre_prod_semiterminado") + "&nbsp;</td>"+
                                             "<td    >" +(rs.getInt("cod_presentacion")>0?rs.getInt("cod_presentacion"):"")+ "&nbsp;</td>"+
                                             "<td    >"+rs.getString("nombrePresentacion")+"&nbsp;</td>";
                            if(rs.getInt("cod_salida_almacen")!=codSalidaAlmacen)
                            {
                                if(codSalidaAlmacen>0)
                                {
                                    rs.previous();
                                    consultaApt="select iv1.NRO_INGRESOVENTAS,av.NOMBRE_ALMACEN_VENTA,pp1.NOMBRE_PRODUCTO_PRESENTACION,"+
                                        ...
                                        innerHtmlApt="<td colspan='5' style='padding:0px !important' rowspan='"+contSalidaAlmacen+"'><table cellpadding='0' cellspacing='0'>";
                                        contFilasApt=0;
                                        while(resApt.next())
                                        {
                                            innerHtmlApt+=(resApt.getRow()==0?"":"<tr>")+
                                        ...
                                        innerHtmlApt+="</table></td></tr>";
                                    out.println(innerHTMLInicio+innerHtmlApt+innerHTMLFinal);
                                    rs.next();
                                }
                                innerHTMLInicio=innerHTMLTemporal;
                                innerHTMLFinal="";
                            ...
                            else
                            {
                                ...
                                innerHTMLFinal+=innerHTMLTemporal+"</tr>";
                            }
                            contSalidaAlmacen++;
                        ...
                        if(codSalidaAlmacen>0)
                        {
                            ...
                            consultaApt="select iv1.NRO_INGRESOVENTAS,av.NOMBRE_ALMACEN_VENTA,pp1.NOMBRE_PRODUCTO_PRESENTACION,"+
                                ...
                                System.out.println("consulta ingresos apt "+consultaApt);
                                resApt=stApt.executeQuery(consultaApt);
                                innerHtmlApt="<td style='padding:0px !important' rowspan='"+contSalidaAlmacen+"'><table cellpadding='0' cellspacing='0'>";
                                contFilasApt=0;
                                while(resApt.next())
                                {
                                    innerHtmlApt+=(resApt.getRow()==0?"":"<tr>")+
                        ...
                        if(conFecha)
                        {
                             String devoluciones = " select ISNULL((select top 1 cp.nombre_prod_semiterminado from COMPONENTES_PROD cp where cp.COD_COMPPROD=sa.COD_PROD),'') AS nombreProd,"+
                                    ...
                                    out.print("<tr>");
                                    out.print("<td    align='right'   >"+rs_devoluciones.getString("NRO_SALIDA_ALMACEN")+"</td>");
                                    out.print("<td   align='right'   >"+rs_devoluciones.getString("NOMBRE_TIPO_SALIDA_ALMACEN")+"</td>");
                                    out.print("<td   align='right'   >"+sdf2.format(rs_devoluciones.getTimestamp("FECHA_SALIDA_ALMACEN"))+"</td>");
                                    out.print("<td   align='right'   >"+rs_devoluciones.getString("COD_LOTE_PRODUCCION")+"</td>");


                                    out.print("<td   align='right'   >"+rs_devoluciones.getString("CODIGO_ANTIGUO")+"</td>");
                                    out.print("<td   align='right'   >"+rs_devoluciones.getString("NOMBRE_MATERIAL")+"</td>");
                                    out.print("<td   align='right'   >"+rs_devoluciones.getString("NOMBRE_CAPITULO")+"</td>");
                                    out.print("<td   align='right'   >"+rs_devoluciones.getString("NOMBRE_GRUPO")+"</td>");
                                    out.print("<td   align='right'   >"+rs_devoluciones.getString("ABREVIATURA")+"</td>");
                                    out.print("<td   align='right'   >0.00</td>");
                                    out.print("<td   align='right'   >0.00</td>");
                                    out.print("<td   align='right'   >0.00</td>");
                                    out.print("<td  ALIGN='RIGHT'   >" + (formato.format(cantDev)) + "</td>");
                                    out.print("<td  ALIGN='RIGHT'   >" + (redondear(costoUnitDev, 4)) + "</td>");
                                    out.print("<td  ALIGN='RIGHT'   >" + (formato.format(costoTotalDev)) + "</td>");
                                    out.print("<td    align='right'   >-"+(formato.format(cantDev))+"</td>");
                                    out.print("<td    align='right'   >"+redondear(costoUnitDev, 4)+"</td>");
                                    out.print("<td    align='right'   >-"+formato.format(costoTotalDev)+"</td>");
                                    out.print("<td   align='right'   >"+rs_devoluciones.getString("nombreProd")+"</td>");
                                    out.print("<td   align='right'   >"+presentacion[0]+"</td>");
                                    out.print("<td   align='right'   >"+presentacion[1]+"</td>");
