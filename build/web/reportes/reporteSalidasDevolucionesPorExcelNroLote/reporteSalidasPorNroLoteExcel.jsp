            ...
            <table width="90%" align="center" class="outputText0" style="border : solid #000000 1px;" cellpadding="0" cellspacing="0" >
                ...
                for(String codLote:arrayLotes)
                {
                    ponerColor=true;
                        ...
                        salida+=" ISNULL(cp.nombre_prod_semiterminado,'') AS nombre_prod_semiterminado,ISNULL((select top 1 cast(pp.cod_anterior as varchar)+'&&&'+cast(pp.NOMBRE_PRODUCTO_PRESENTACION as varchar) from PRESENTACIONES_PRODUCTO pp where pp.cod_presentacion=sa.cod_presentacion ),' &&& ') as datosPresentacion";
                        ...
                        if(conFecha)
                        {
                             String devoluciones = " select ISNULL((select top 1 cp.nombre_prod_semiterminado from COMPONENTES_PROD cp where cp.COD_COMPPROD=sa.COD_PROD),'') AS nombreProd,"+
                                                      " ISNULL((select top 1 cast(pp.cod_anterior as varchar )+'&&&'+cast(pp.NOMBRE_PRODUCTO_PRESENTACION as varchar) from PRESENTACIONES_PRODUCTO pp where pp.cod_presentacion=sa.COD_PRESENTACION),' &&& ')as nombreProducto, d.cod_devolucion,sa.cod_salida_almacen,";
                                    devoluciones += " ia.fecha_ingreso_almacen,ia.cod_ingreso_almacen,";
                                    devoluciones += " iad.cod_material,";
                                    devoluciones += " isnull(iad.CANT_TOTAL_INGRESO_FISICO,0)as CANT_TOTAL_INGRESO_FISICO,";
                                    devoluciones += " isnull(iad.COSTO_UNITARIO_ACTUALIZADO,0)as COSTO_UNITARIO_ACTUALIZADO,";
