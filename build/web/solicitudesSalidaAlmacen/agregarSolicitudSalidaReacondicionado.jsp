...
<f:view>
    ...
    <html>
       ...
       {
           if(parseInt(document.getElementById("form1:codPresentacion").value) == 0)
           {
           ...
           {
               if(parseFloat(tabla.rows[i].cells[1].getElementsByTagName("input")[0].value) <= 0)
               {
                   alert('la cantidad a solicitar no puede ser menor o igual a 0');
                   return false;
               }
           }
           return true;
        ...
        <body onload="onLoad()">
            <a4j:form id="form1">
                <div align="center">
                    ...
                    <h:outputText value="#{ManagedRegistroSolicitudSalidaAlmacen.cargarRegistroSolicitudSalidaReacondicionamiento}"/>
                    <br><br>
                    ...
                    <rich:panel style="width:100%"  headerClass="headerClassACliente" id="datosSolicitud">
                            <f:facet name="header">
                                <h:outputText value="Registro de Solicitud Salidas a Almacen"  />
                            </f:facet>
                        <h:panelGrid columns="2" style="width:100%">
                            <h:outputText value="Presentacion:" styleClass="outputText2"/>
                            <h:selectOneMenu  styleClass="inputText" id="codPresentacion" value="#{ManagedRegistroSolicitudSalidaAlmacen.solicitudesSalidaReacondicionamiento.presentacionesProducto.codPresentacion}">
                                <f:selectItem itemValue="0" itemLabel="--Seleccione una opción--"/>
                                <f:selectItems value="#{ManagedRegistroSolicitudSalidaAlmacen.presentacionesProductoSelectList}"  />
                                <a4j:support event="onchange" action="#{ManagedRegistroSolicitudSalidaAlmacen.codPresentacionSolicitudReacondicionamiento_change}"
                                             reRender="dataSalidasAlmacenDetalle,datosSolicitud"/>
                            </h:selectOneMenu>
                            <h:outputText value="Area / Departamento:" styleClass="outputText2" />
                            <h:outputText value="#{ManagedRegistroSolicitudSalidaAlmacen.solicitudesSalidaReacondicionamiento.areaDestinoSalida.nombreAreaEmpresa}"
                                          styleClass="outputText2"/>
                            
                            <h:outputText value="Solicitante:" styleClass="outputText2" />
                            <h:panelGroup>
                                <h:outputText styleClass="outputText2" value="#{ManagedAccesoSistema.usuarioModuloBean.nombrePilaUsuarioGlobal}" /> &nbsp;                            
                                <h:outputText styleClass="outputText2" value="#{ManagedAccesoSistema.usuarioModuloBean.apPaternoUsuarioGlobal}" /> &nbsp;
                                <h:outputText styleClass="outputText2" value="#{ManagedAccesoSistema.usuarioModuloBean.apMaternoUsuarioGlobal}" />
                            </h:panelGroup>
                    ...
                    <br/>
                     <div style="overflow:auto;height:200px;width:80%;top:10px;text-align:center">
                         <rich:dataTable value="#{ManagedRegistroSolicitudSalidaAlmacen.solicitudesSalidaReacondicionamiento.solicitudesSalidaDetalleList}"                                var="data"
                        ...
                        <rich:column>
                            ...
                            <h:inputText value="#{data.cantidad}" styleClass="inputText"/>
                        </rich:column>
                        ...
                        <rich:column>
                            ...
                            <a4j:commandButton value="Eliminar" styleClass="btn"
                                               ...
                                               action="#{ManagedRegistroSolicitudSalidaAlmacen.eliminarSolicitudDetalleReacond_action}">
                                <f:setPropertyActionListener value="#{data}" target="#{ManagedRegistroSolicitudSalidaAlmacen.solicitudesSalidaDetalleEliminar}"/>
                            </a4j:commandButton>
                        </rich:column>
                    </rich:dataTable>
                    </div>
                    ...
                    <a4j:commandButton value="Guardar" styleClass="btn"
                                       action="#{ManagedRegistroSolicitudSalidaAlmacen.guardarSolicitudSalidaReacondicionamiento_action}"
                                onclick="if(validarFormularioRegistro()){if(confirm('Esta seguro de realizar la Solicitud de Salida de Almacen?')==false){return false;}}else{return false;}"
                                oncomplete="if(#{ManagedRegistroSolicitudSalidaAlmacen.mensaje eq '1'}){alert('Se registro satisfactoriamente la solicitud');window.location.href='navegadorSolicitudesSalidaAlmacen.jsf?data='+(new Date()).getTime().toString();}
                                else {alert('#{ManagedRegistroSolicitudSalidaAlmacen.mensaje}');}"/>
                    <a4j:commandButton value="Cancelar" styleClass="btn" oncomplete="window.location.href='navegadorSolicitudesSalidaAlmacen.jsf?data='+(new Date()).getTime().toString();"/>
